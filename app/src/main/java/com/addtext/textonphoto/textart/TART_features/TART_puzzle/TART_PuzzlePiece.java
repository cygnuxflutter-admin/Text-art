...
public class TART_PuzzlePiece {
    ...
    TART_PuzzlePiece(Drawable drawable2, TART_Area area2, Matrix matrix2) {
    ...
    private float getMatrixScale() {
        return TART_MatrixUtils.getMatrixScale(this.matrix);
    }


    public float getMatrixAngle() {
        return TART_MatrixUtils.getMatrixAngle(this.matrix);
    }
    ...
    public boolean canFilledArea() {
        return TART_MatrixUtils.getMatrixScale(this.matrix) >= TART_MatrixUtils.getMinMatrixScale(this);
    }
    ...
    public void postRotate(float f) {
        ...
        float minMatrixScale = TART_MatrixUtils.getMinMatrixScale(this);
        ...
        if (!TART_MatrixUtils.judgeIsImageContainsBorder(this, getMatrixAngle())) {
            float[] calculateImageIndents = TART_MatrixUtils.calculateImageIndents(this);
    ...
    private void animateTranslate(final View view, final float f, final float f2) {
        ...
        this.animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
                TART_PuzzlePiece.this.translate(f * ((Float) valueAnimator.getAnimatedValue()).floatValue(), f2 * ((Float) valueAnimator.getAnimatedValue()).floatValue());
                view.invalidate();
            }
        });
    ...
    public void fillArea(View view, boolean z) {
        if (!isFilledArea()) {
            ...
            final float minMatrixScale = TART_MatrixUtils.getMinMatrixScale(this);
            ...
            this.animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                public void onAnimationUpdate(ValueAnimator valueAnimator) {
                    ...
                    TART_PuzzlePiece.this.zoom(f, f, pointF);
                    TART_PuzzlePiece.this.postTranslate(f2, f3);
                    view2.invalidate();
                }
            });
            if (z) {
    ...
    public void updateWith(MotionEvent motionEvent, TART_Line line) {
        float x = (motionEvent.getX() - this.previousMoveX) / 2.0f;
        float y = (motionEvent.getY() - this.previousMoveY) / 2.0f;
        if (!canFilledArea()) {
            TART_Area area2 = getArea();
            float minMatrixScale = TART_MatrixUtils.getMinMatrixScale(this) / getMatrixScale();
        ...
        if (line.direction() == TART_Line.Direction.HORIZONTAL) {
            translate(0.0f, y);
        } else if (line.direction() == TART_Line.Direction.VERTICAL) {
            translate(x, 0.0f);
        }
        RectF currentDrawableBounds = getCurrentDrawableBounds();
        TART_Area area3 = getArea();
    ...
    public void setArea(TART_Area area2) {
        this.area = area2;
    }
