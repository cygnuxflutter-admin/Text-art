...
import com.addtext.textonphoto.textart.TART_photoeditor.TART_BrushDrawingView;
...
public class TART_MosaicView extends ImageView {
    ...
    private Stack<TART_BrushDrawingView.LinePath> lstPoints = new Stack<>();
    private Path mPath;
    private Stack<TART_BrushDrawingView.LinePath> mPoints = new Stack<>();
    private Stack<TART_BrushDrawingView.LinePath> mRedoPaths = new Stack<>();
    private float mTouchX;
    private float mTouchY;
    private TART_MosaicAdapter.MosaicItem mosaicItem;
    private boolean showTouchIcon = false;

    public void setMosaicItem(TART_MosaicAdapter.MosaicItem mosaicItem2) {
        this.mosaicItem = mosaicItem2;
        if (mosaicItem2.mode == TART_MosaicAdapter.Mode.SHADER) {
    ...
    public TART_MosaicView(Context context) {
        super(context);
        init();
    }

    public TART_MosaicView(Context context, AttributeSet attributeSet) {
        super(context, attributeSet);
        init();
    }

    public TART_MosaicView(Context context, AttributeSet attributeSet, int i) {
    ...
    private void init() {
        ...
        this.circlePaint.setStrokeWidth((float) TART_SystemUtil.dpToPx(getContext(), 2));
    ...
    public void onDraw(Canvas canvas) {
        ...
        while (it.hasNext()) {
            TART_BrushDrawingView.LinePath linePath = (TART_BrushDrawingView.LinePath) it.next();
            canvas.drawPath(linePath.getDrawPath(), linePath.getDrawPaint());
        }
        if (this.mosaicItem.mode == TART_MosaicAdapter.Mode.BLUR || this.mosaicItem.mode == TART_MosaicAdapter.Mode.MOSAIC) {
    ...
    public void onTouchUp(@NonNull MotionEvent motionEvent) {
        TART_BrushDrawingView.LinePath linePath;
        this.showTouchIcon = false;
        if (this.mosaicItem.mode == TART_MosaicAdapter.Mode.BLUR || this.mosaicItem.mode == TART_MosaicAdapter.Mode.MOSAIC) {
            linePath = new TART_BrushDrawingView.LinePath(this.mPath, this.blurPaint);
        } else {
            linePath = new TART_BrushDrawingView.LinePath(this.mPath, this.bitmapPaint);
        }
...
//        int actionMasked = MotionEventCompat.getActionMasked(motionEvent);
    ...
    public boolean undo() {
        if (!this.lstPoints.empty()) {
            TART_BrushDrawingView.LinePath pop = this.lstPoints.pop();
    ...
    public boolean redo() {
        if (!this.mRedoPaths.empty()) {
            TART_BrushDrawingView.LinePath pop = this.mRedoPaths.pop();
