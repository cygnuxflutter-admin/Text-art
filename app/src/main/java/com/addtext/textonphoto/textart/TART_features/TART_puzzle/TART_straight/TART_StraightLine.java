...
class TART_StraightLine implements TART_Line {
    TART_StraightLine attachLineEnd;
    TART_StraightLine attachLineStart;
    private RectF bounds = new RectF();
    public TART_Line.Direction direction = TART_Line.Direction.HORIZONTAL;
    private PointF end;
    private float endRatio;
    private TART_Line lowerLine;
    private PointF previousEnd = new PointF();
    private PointF previousStart = new PointF();
    private PointF start;
    private float startRatio;
    private TART_Line upperLine;

    TART_StraightLine(PointF pointF, PointF pointF2) {
        this.start = pointF;
        this.end = pointF2;
        if (pointF.x == pointF2.x) {
            this.direction = TART_Line.Direction.VERTICAL;
        } else if (pointF.y == pointF2.y) {
            this.direction = TART_Line.Direction.HORIZONTAL;
        } else {
    ...
    public void setAttachLineStart(TART_StraightLine straightLine) {
        this.attachLineStart = straightLine;
    }


    public void setAttachLineEnd(TART_StraightLine straightLine) {
    ...
    public float slope() {
        return this.direction == TART_Line.Direction.HORIZONTAL ? 0.0f : Float.MAX_VALUE;
    }
    ...
    public boolean contains(float f, float f2, float f3) {
        ...
        } else if (this.direction == TART_Line.Direction.VERTICAL) {
